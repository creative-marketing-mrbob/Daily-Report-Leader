...
export function periodGrowthAtDate(name:string,reports:SavedReport[],date:string){
 ...
 return {actual:current.value-first.value,baseline:current.date===first.date,baselineDate:first.date};
}
