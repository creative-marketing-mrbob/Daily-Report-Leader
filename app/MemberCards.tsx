...
export default function MemberCards({names,reports,scoreReports,month}:{names:string[];reports:SavedReport[];scoreReports:SavedReport[];month:string}){
 ...
 const mixed=targets.some(t=>['Total pertumbuhan followers','Total penambahan likes TikTok','Total lead yang dihasilkan'].includes(t.title));
 ...
 {['Dewi','Ilham','Mario'].includes(name)&&<div className="leader-result"><span>{name==='Dewi'?'Pertumbuhan followers':name==='Mario'?'Likes TikTok':'Jumlah lead'}</span><strong>{latest&&latest.metric!==''?Number(latest.metric).toLocaleString('id-ID'):'—'}</strong></div>}
