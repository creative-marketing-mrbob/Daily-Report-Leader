...
export default function MemberReportPanels({names,reports,scoreReports,month}:{names:string[];reports:SavedReport[];scoreReports:SavedReport[];month:string}){
 ...
 const monthlyTasks=scoreReports.filter(r=>r.date.startsWith(month)).flatMap(r=>r.members.filter(m=>m.name===name).flatMap(m=>m.tasks));
 const requests=Array.from(new Set([...(categories[name]||[]),...monthlyTasks.map(t=>t.kategori)])).filter(label=>/request/i.test(label));
 const summaries=[...targets.map(t=>({label:shortTitles[t.title]||t.title,value:`${t.actual===null?'—':t.actual.toLocaleString('id-ID')} / ${t.target.toLocaleString('id-ID')}`,title:`${t.title}: hasil / target 28 hari kerja`})),...requests.map(label=>({label,value:`${monthlyTasks.filter(t=>t.kategori===label&&Number(t.progress)===100).length} / ${monthlyTasks.filter(t=>t.kategori===label).length}`,title:'Request: selesai / jumlah request dilaporkan; tidak termasuk skor target'}))];
 return <section className="member-report-panel" key={name}><header className="member-panel-heading"><h3>{name}</h3><span>Skor <strong>{score===null?'—':`${score}%`}</strong></span></header><div className="member-target-strip" aria-label={`Target ${name}`}>{summaries.map(s=><div className="member-target-item" key={s.label} title={s.title}><span>{s.label}</span><strong>{s.value}</strong></div>)}</div><p className="member-target-caption">Hasil / target · {month} · 28 hari kerja{requests.length>0?' · Request: selesai / dilaporkan':''}</p><div className="member-activities-scroll"><table className="member-activities-table"><caption className="sr-only">Aktivitas harian {name} dari report leader</caption><thead><tr><th scope="col">Aktivitas</th><th scope="col">Kategori</th><th scope="col">Progress</th></tr></thead><tbody>{entries.length?entries.map(entry=><DayRows key={entry.date} entry={entry}/>):<tr><td colSpan={3} className="member-empty">Belum ada aktivitas dari report leader pada periode ini.</td></tr>}</tbody></table></div></section>;
 })}</div>;
}
